...
public class BasicCOLA<K extends Comparable<K>, V> {
     ...
     */
    public static int DISK_BLOCK_SIZE = 40; // in real applications this would be larger, e.g. 4096
    ...
    public BasicCOLA(int elementSize, final Converter<K> keyConverter, final Converter<V> valueConverter, Container rawContainer) {
        ...
        this.container = new ConverterContainer(rawContainer, new Converter<COLABlock<K, V>>() {
            ...
            public COLABlock<K, V> read(DataInput dataInput, COLABlock<K, V> block) throws IOException {
                ...
                int size = dataInput.readInt(); // Number of elements in the block
                block = new COLABlock<>(size);  // Initialize the block
            ...
            public void write(DataOutput dataOutput, COLABlock<K, V> object) throws IOException {
                ...
                dataOutput.writeInt(object.getSize()); // Write the number of elements
    ...
    public V searchElement(K key) throws NoSuchElementException {
        ...
        // Iterate through all levels, from the top to the bottom
        for (int level = 0; level < arrayOffsets.size(); level++) {
            // Retrieve the level dynamically using getLevel
            COLALevel<K, V> colaLevel = getLevel(level);

            // Perform the search in the current level
            V value = colaLevel.search(key);
            if (value != null) {
                return value; // Key found, return the value
            }
        }

        // If the key is not found in any level, throw an exception
